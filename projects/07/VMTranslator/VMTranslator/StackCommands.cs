...
namespace VMTranslator
{
    public static class StackCommands
    {
        ...
        {
            string msg = "//Incrementing Stack Pointer\n" +
               $"@{pointerName}\n" +
               "M=M+1\n";
            return msg;
        }
        ...
        {
            string msg = $"//Popping to {location} stack\n";
        ...
        {
            string msg = $"//Pushing {value} to stack\n" +
                $"@{value}\n" +
                $"D=A\n" +
                $"@SP       //SP = {value}\n" +
                $"A=M\n" +
        ...
        public static string Push(int value, string keyword)
        {
            string msg = $"//Pushing {value} to {keyword} stack\n" +
                $"@{keyword}\n" +
                $"D=M\n" +
                PushFromDReg() +
                Push(value) +
        ...
        public static string PushFromMemory(CommandMemoryLocation location, int value)
        {
            int pointerLocation = memLocDictionary[location];
            string msg = $"//Pushing {value} to {location} stack\n";
            msg += $"@{pointerLocation}\n";
            msg += SetDFromM();
            msg += PushFromDReg();
            msg += Push(value);
        ...
        {
            int address = 5 + value;
            string msg = "//Pop to temp stack\n";
            msg += Pop();
            msg += SetDFromM();
            msg += $"@{address}     //Temp pointer\n";
            msg += "M=D";
            return msg;
        }
        ...
        {
            int address = 5 + value;
            string msg = $"//Pushing from temp stack\n";
            msg += $"@{address}\n";
            msg += SetDFromM();
            msg += PushFromDReg();
            return msg;
        ...
        {
            string msg = $"//Pushing {varName} to stack\n" +
                $"@{varName}\n" +
                $"D=M\n" +
                PushFromDReg();
            return msg;
        ...
        {
            return "@SP     //Pushing from D Register to main stack\n" +
                "A=M\n" +
